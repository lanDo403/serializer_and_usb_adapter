...
Внутренние потоки связаны AXI-Stream-подобным контрактом: `valid`, `ready`, `data`, `keep`. Передача слова происходит при `valid && ready`. Источник держит `data/keep` стабильными, пока приемник не подтвердил передачу через `ready`. `keep[3:0]` соответствует `BE[3:0]`, а `data[31:0]` соответствует `DATA[31:0]`.
...
GPIO-домен пишет слова в normal TX FIFO. FT-домен читает их, когда arbiter выбирает normal source и FT601 готов принять данные. FT601 RX path остается активным для service-команд.
...
Loopback FIFO хранит 36 бит на слово: `{BE[3:0], DATA[31:0]}`. Это сохраняет byte-enable информацию. Командные слова service frame в эту FIFO не записываются.
